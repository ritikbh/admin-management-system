...
const submitKycData = () =>{
    ...
    axios({
        method: 'post',
        url: 'http://localhost:8000/api/insert-kyc',
        data: {name:userName,
              email:emailId
        } ,
...
const deleteClient = (e,userId) => {

    e.preventDefault();
    // console.log(userId);
    axios({
        method: 'post',
        url: 'http://localhost:8000/api/delete-specific-data',
        data: {user_id:userId} ,
        headers: {'Content-Type': 'application/json' }
        })
        .then(function (response) {
...
const handleClose = () =>{
    ...
    return (
        <div>
                 <Modal show={kycShow} onHide={handleClose}>
   
   <Modal.Body>

<Form.Group>
<Form.Label><b>Enter KYC Admin Name</b></Form.Label>
...
<Button variant="success" onClick={submitKycData}>
Submit
</Button>
...
Submit
 ...
 <div className="col-12">
            ...
            <div className="row">
            <div className="col-3">
            <button className="btn btn-primary" onClick={showKycModal}>Create KYC Admin</button>
            </div>
            <div className="col-3">
...
<CSVLink filename={"Client-table.csv"} className="btn btn-success" data={tableData} >Export CSV</CSVLink>
  ...
  <tbody>
    ...
    <td><button className="btn btn-danger" onClick={(e) => deleteClient(e,item.id)}>Delete</button></td>
...
<CSVLink filename={"Log-table.csv"} className="btn btn-success" data={logTableData} >Export CSV</CSVLink>
